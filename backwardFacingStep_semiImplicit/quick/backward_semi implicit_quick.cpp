...
void velocity_convective1_quick(int i,int j)
{
                ...
                if(Fe>=0)
                {
                    C1=Fe*(6*ust[i][j]+3*ust[i+1][j]-1*ust[i-1][j])/8;
                    F=(3*Fe)/4;
                }
                else
                {
                    C1=Fe*(6*ust[i+1][j]+3*ust[i][j]-1*ust[i+2][j])/8;
                   F=(3*Fe)/8;
                }
                 Fw=((un[i-1][j]+un[i][j])/2)*(-1*dy);
                if(Fw>=0)
                {
                    ...
                    C2=Fw*(6*ust[i][j]+3*ust[i-1][j]-1*ust[i+1][j])/8;
                    F=F+(3*Fw)/4;
                }
                else
                {
                    C2=Fw*(6*ust[i-1][j]+3*ust[i][j]-1*ust[i-2][j])/8;
                     F=F+(3*Fw)/8;
                }
                Fn=((vn[i][j+1]+vn[i][j])/2)*dx;
                if(Fn>=0)
                {
                    C3=Fn*(6*ust[i][j]+3*ust[i][j+1]-1*ust[i][j-1])/8;
                    F=F+(3*Fn)/4;
                }
                else
                {
                    C3=Fn*(6*ust[i][j+1]+3*ust[i][j]-1*ust[i][j+2])/8;
                     F=F+(3*Fn)/8;
                }
                Fs=((vn[i][j-1]+vn[i][j])/2)*(-1*dx);
                if(Fs>=0)
                {
                    ...
                    C4=Fs*(6*ust[i][j]+3*ust[i][j-1]-1*ust[i][j+1])/8;
                   F=F+(3*Fs)/4;
                }
                else
                {
                     C4=Fs*(6*ust[i][j-1]+3*ust[i][j]-1*ust[i][j-2])/8;
                      F=F+(3*Fs)/8;
                }
                Cu=C1+C2+C3+C4;
...
void velocity_convective2_quick(int i,int j)
{
                ...
                if(Fe>=0)
                {
                    C1=Fe*(6*vst[i][j]+3*vst[i+1][j]-1*vst[i-1][j])/8;
                    F=(3*Fe)/4;
                }
                else
                {
                    C1=Fe*(6*vst[i+1][j]+3*vst[i][j]-1*vst[i+2][j])/8;
                   F=(3*Fe)/8;
                }
                 Fw=((un[i-1][j]+un[i][j])/2)*(-1*dy);
                if(Fw>=0)
                {
                    ...
                    C2=Fw*(6*vst[i][j]+3*vst[i-1][j]-1*vst[i+1][j])/8;
                    F=F+(3*Fw)/4;
                }
                else
                {

                    C2=Fw*(6*vst[i-1][j]+3*vst[i][j]-1*vst[i-2][j])/8;
                    F=F+(3*Fw)/8;
                }
                Fn=((vn[i][j+1]+vn[i][j])/2)*dx;
                if(Fn>=0)
                {
                    C3=Fn*(6*vst[i][j]+3*vst[i][j+1]-1*vst[i][j-1])/8;
                  F=F+(3*Fn)/4;
                }
                else
                {
                    C3=Fn*(6*vst[i][j+1]+3*vst[i][j]-1*vst[i][j+2])/8;
                    F=F+(3*Fn)/8;
                }
                    Fs=((vn[i][j-1]+vn[i][j])/2)*(-1*dx);
                if(Fs>=0)
                {
                    ...
                    C4=Fs*(6*vst[i][j]+3*vst[i][j-1]-1*vst[i][j+1])/8;
                    F=F+(3*Fs)/4;
                }
                else
                {
                     C4=Fs*(6*vst[i][j-1]+3*vst[i][j]-1*vst[i][j-2])/8;
                     F=F+(3*Fs)/8;
                }
                Cv=C1+C2+C3+C4;
